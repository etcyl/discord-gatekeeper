...
@bot.command()
async def addalt(ctx, alt_name: str):
    user_id = str(ctx.author.id)
    alts = alts_data.get(user_id, [])
    ...
    alts_data[user_id] = {"alts": alts, "main": alts_data.get(user_id, {}).get("main")}
    save_alts(alts_data)
    await ctx.send(f"✅ Added alt `{alt_name}` to your account.")
