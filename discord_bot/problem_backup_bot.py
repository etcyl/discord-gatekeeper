...
# === Current-Week Raid Mirror: single-drop snippet (no RAID_HELPER_BOT_ID) ===
# Paste this anywhere in your existing bot file after imports.
# Then call: await register_raid_mirror(bot) once at startup (e.g., in on_ready).
import re
import json
...
# CONFIG (edit these)
# -----------------------
# Map source channel name -> logical raid key (exactly one mirror per key in destination)
SOURCE_CHANNELS: Dict[str, str] = {
    ...
    # add more: "naxx-wed-sign-up": "NAXX-Wednesday",
}

# Destination channel that must contain only the current week’s sign-ups
DESTINATION_CHANNEL: str = "current-raids"

# Minimal persistent state so mirrors survive restarts
...
def _iso_week_key(now: Optional[datetime] = None) -> str:
    """Return 'YYYY-Www' for current ISO week (UTC)."""
    now = (now or datetime.now(timezone.utc)).astimezone(timezone.utc)
    year, week, _ = now.isocalendar()
    return f"{year}-W{week:02d}"

def _normalize(name: str) -> str:
    """Normalize channel names for dict lookup."""
    return re.sub(r"[\s_]+", "-", name.strip().lower())

def _find_channel_by_name(guild: discord.Guild, name: str) -> Optional[discord.TextChannel]:
    """Find a text channel by name, allowing minor spacing/underscore variations."""
...
def _clone_embed(src: discord.Embed) -> discord.Embed:
    """Best-effort visual clone of a Discord embed (non-interactive)."""
    dst = discord.Embed(
        title=src.title,
...
def _load_state() -> dict:
    """
    Return persisted mirror state:
      {"week_key": "YYYY-Www", "mirrors": {"AQ40-Friday": {"source_msg_id": 123, "dest_msg_id": 456}, ...}}
    """
    try:
        ...
        pass
    return {"week_key": _iso_week_key(), "mirrors": {}}
...
def _raidkey_for_message(message: discord.Message) -> Optional[str]:
    """Return raid key if message is from a configured source channel and has an embed."""
...
class CurrentWeekRaidMirror(commands.Cog):
    """
    Mirrors the newest Raid-Helper embed from each configured source channel into
    #current-raids, keeping exactly one mirror per raid type for the current ISO week.
    Replaces the mirror when a newer event appears, and updates the mirror if the
    original message is edited. On week change, clears old mirrors automatically.
    """
    ...
    async def _ensure_week(self, guild: discord.Guild) -> None:
        """If the week changed, delete previous mirrors and reset state."""
    ...
    async def refresh_all_mirrors(self, guild: discord.Guild, per_channel_scan: int = 50) -> None:
        """
        Backfill/refresh the destination channel (#current-raids) with the newest
        Raid-Helper embed from each configured source channel for the current ISO week.

        Args:
            guild: The guild to operate on.
            per_channel_scan: How many recent messages to scan in each source channel.
        """
        try:
            ...
            for src_name, raidkey in SOURCE_CHANNELS.items():
                ...
                # Find most recent message with an embed that qualifies as a raid post
                found = None
                ...
                if found:
                    # Replace existing mirror for this raidkey with the newest one we found
                    await self._post_or_replace(guild, raidkey, found)
        except Exception:
            # Silent guard; you already have logging for inner ops
            pass


    async def _post_or_replace(self, guild: discord.Guild, raidkey: str, source_msg: discord.Message) -> None:
        """Post or replace the mirror for this raid type in destination channel."""
        ...
        # delete old mirror for this key, if any
    ...
    async def _update_mirror(self, guild: discord.Guild, raidkey: str, source_msg: discord.Message) -> None:
        """Edit the existing mirror to match the updated source message."""
    ...
    async def on_message_create(self, message: discord.Message) -> None:
        """Mirror new event embeds from configured source channels."""
        try:
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message) -> None:
        """Keep mirrors updated when source messages are edited."""
        try:
            if not after.guild:
                return
...
async def register_raid_mirror(bot: commands.Bot) -> None:
    """Call this once during startup to enable the mirroring Cog."""
...
# === COMMAND: ADD ALT ===
# === COMMAND: ADD ALT (alt_class optional, validated in-command) ===
@bot.command()
async def addalt(ctx, alt_name: str, *, alt_class: str = None):
    ...
    Usage:
        !addalt <alt_name> <class>
    Example:
        !addalt Shadowbolt Warlock
    """
    try:
        if not alt_class:
            await ctx.send(
...
async def on_ready():
    ...
    Also refreshes the current-raids channel from source channels on boot.
    """
    ...
    if not hasattr(bot, "_raid_mirror_loaded"):
        ...
        # NEW: perform a boot-time refresh for each guild
        cog = bot.get_cog("CurrentWeekRaidMirror")
        if cog:
            for g in bot.guilds:
                await cog.refresh_all_mirrors(g)

    # existing onboarding checks
    for guild in bot.guilds:
...
async def refresh_raids_command(ctx: commands.Context, scan: int = 50):
    """
    Refresh the #current-raids channel with the latest raid sign-ups from all
    configured source channels. Default scan depth is the 50 most recent messages
    per source channel. Increase with: !refreshraids 200
    """
    try:
        cog = bot.get_cog("CurrentWeekRaidMirror")
        if not cog:
        ...
        # discord.py will normally handle this, but keep a friendly message
        await ctx.send("You do not have permission to run this command.")
    except Exception as e:
        # keep user feedback terse, errors go to your logs
